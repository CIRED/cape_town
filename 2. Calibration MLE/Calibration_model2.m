function [score_amenities, score_hous, error_income, indexes, parameters_amenities, model_amenity, which_rent] = ...
    Calibration_model2(net_income, data_rent, data_density, data_dwelling_size, data_income_class, ...
...
% Calculation for dwelling sizes
...
indexes_param = combvec(list_rho, list_beta, list_basic_q);
...
for index_param = 1:size(indexes_param,2)
        ...
        rho = indexes_param(1, index_param);
        beta = indexes_param(2, index_param);
        q_0 = indexes_param(3,index_param);
                        
        % Error on dwelling sizes
        hous = calcule_hous2(indexes_param(:,index_param), net_income_hous, data_rent_which);
        ...
        R_interp = calcule_rent(beta, q_0, net_income_rent, XX, ZZ_R);
        
        list_U = combvec(list_Uo3, list_Uo4);
        
        for index_U = 1:size(list_U,2)
            
            index_tot = index_tot + 1;
            
            indexes(:, index_tot) = [rho; beta; q_0; list_Uo2; list_U(:,index_U)];
            Uo = indexes(4:6, index_tot);
